...
const SilkScene=lazy(()=>import('../components/SilkScene').then(module=>({default:module.SilkScene})));
...
const LoginTitle=memo(function LoginTitle(){
...
export function Login(){
  ...
  return <div className="login"><section className="login-brand"><Suspense fallback={null}><SilkScene/></Suspense><div className="login-copy"><img src="/brand/konooz-wordmark-transparent.png" alt="Konooz"/><p>THE STYLE YOU LOVE</p><LoginTitle/><span>Inventory, sales and every treasured detail, securely connected to the shop database.</span></div></section><section className="login-form"><form onSubmit={submit}><div className="lock"><LockKeyhole size={20}/></div><p className="eyebrow">PRIVATE ACCESS</p><h2>Welcome back</h2><p className="muted">Sign in to open the Konooz shop desk.</p><label>Username<input autoFocus autoComplete="username" autoCapitalize="none" spellCheck={false} value={username} onChange={event=>setUsername(event.target.value)} required/></label><label>Password<input type="password" autoComplete="current-password" value={password} onChange={event=>setPassword(event.target.value)} required/></label>{error&&<div className="error" role="alert">{error}</div>}<button className="primary login-submit" disabled={busy}>{busy?<><LoaderCircle className="spin"/>Opening atelier...</>:<>Enter Konooz<ArrowRight size={18}/></>}</button><small>Secure access to the live shop database</small></form></section></div>;
}
