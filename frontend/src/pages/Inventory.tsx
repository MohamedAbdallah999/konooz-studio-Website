...
export function Inventory() {
  ...
  return <>
    ...
    <div className="search"><Search size={19}/><input placeholder="Search model, colour, pack or material..." value={q} onChange={event => setQ(event.target.value)}/><span>{models.length} models</span></div>
    ...
    <AnimatePresence>{editing && <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={event => event.target === event.currentTarget && setEditing(null)}>
      <motion.form className="modal" onSubmit={submit} initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }}>
        <header><div><p className="eyebrow">MODEL CONFIGURATION</p><h2>{models.some(model => model.id === editing.id) ? 'Edit model' : 'New model'}</h2></div><button type="button" className="icon" onClick={() => setEditing(null)} aria-label="Close"><X/></button></header>
        <div className="form-grid">
          <label>Model number<input value={editing.modelNumber} onChange={event => setEditing({ ...editing, modelNumber: event.target.value })} required/></label>
          <label>Base price per size (EGP)<NumberInput min="0" step="0.01" value={editing.price} onChange={event => setEditing({ ...editing, price: event.target.value })} required/></label>
          <label>Model photo<input type="file" accept="image/*" onChange={async event => { const file = event.target.files?.[0]; if (!file) return; try { setEditing({ ...editing, photoUrl: await optimizeModelPhoto(file) }); } catch (error) { alert(error instanceof Error ? error.message : 'Unable to process this image.'); event.target.value = ''; } }}/><small>Up to 15 MB. Large photos are optimized automatically.</small>{editing.photoUrl && <span className="photo-preview"><img src={editing.photoUrl} alt="Selected model preview"/><button type="button" onClick={() => setEditing({ ...editing, photoUrl: '' })}>Remove photo</button></span>}</label>
        ...
        <div className="variants model-colours"><div><div><h3>Colours and packs</h3><small>Stock is the number of available packs. Pack price is calculated from sizes per pack.</small></div><button type="button" onClick={() => setEditing({ ...editing, colours: [...editing.colours, newColour(editing.id)] })}><Plus size={15}/> Add colour</button></div>
          {editing.colours.filter(colour => colour.isActive).map(colour => <section className="colour-editor" key={colour.id}>
            ...
            {colour.packs.filter(pack => pack.isActive).map(pack => <div className="pack-row" key={pack.id}>
              <label>Sizes per pack<NumberInput min="1" step="1" value={pack.sizesPerPack} onChange={event => updatePack(colour.id, pack.id, { sizesPerPack: Number(event.target.value) })} required/></label>
              <label>Packs available<NumberInput min="0" step="1" value={pack.stockQuantity} onChange={event => updatePack(colour.id, pack.id, { stockQuantity: Number(event.target.value) })} required/></label>
