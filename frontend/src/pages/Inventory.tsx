...
export function Inventory() {
  ...
  return <div className="inventory-page">
    ...
    <div className="inventory-grid">{models.map(model => <motion.article layout key={model.id} className="item-card">
      <div className="item-art">{model.photoUrl ? <div className="item-photo-frame" role="img" tabIndex={0} aria-label={`Photo of model ${model.modelNumber}`}><img src={model.photoUrl} alt={`Model ${model.modelNumber}`} draggable={false}/></div> : <span>{model.modelNumber.slice(0, 2).toUpperCase()}</span>}</div>
      <button className="item-edit-button" onClick={() => setEditing(structuredClone(model))} aria-label={`Edit ${model.modelNumber}`}><Pencil size={16}/></button>
