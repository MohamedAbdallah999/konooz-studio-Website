...
export function Dashboard(){
  const data=useLiveQuery(async()=>{
    const models=await db.models.filter(x=>x.isActive).toArray(),packs=await db.packs.filter(x=>x.isActive).toArray(),sales=await db.sales.filter(x=>!x.deletedAt).toArray();
    const today=new Date().toDateString(),todaySales=sales.filter(s=>new Date(s.createdAt).toDateString()===today),revenue=sales.flatMap(paymentEvents).filter(event=>new Date(event.date).toDateString()===today).reduce((sum,event)=>addMoney(sum,event.amount),'0.00');
    return{models:models.length,packs:packs.reduce((sum,pack)=>sum+pack.stockQuantity,0),low:packs.filter(pack=>pack.stockQuantity<=3).length,revenue,sales:todaySales.length};
  })??{models:0,packs:0,low:0,revenue:'0.00',sales:0};
  return <>
    <section className="hero">
      ...
      <div className="hero-figure"><span>Today's revenue</span><strong>{formatMoney(data.revenue)}</strong><small>{data.sales} completed {data.sales===1?'sale':'sales'}</small></div>
    </section>
    <div className="stats">
      <article><Package/><span>Collection</span><strong>{data.models}</strong><small>dress models</small></article>
      <article><ShoppingBag/><span>In the atelier</span><strong>{data.packs}</strong><small>packs available</small></article>
      <article className={data.low?'warn':''}><TriangleAlert/><span>Needs attention</span><strong>{data.low}</strong><small>low-stock packs</small></article>
    </div>
