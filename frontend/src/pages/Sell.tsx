  ...
  return <div className="sell-layout">
    ...
    <aside className="basket">
      ...
      <div className="checkout-details client-details">
        <label>Client name<input autoComplete="name" value={customerName} maxLength={120} onChange={event => setCustomerName(event.target.value)} placeholder="Optional"/></label><label>Phone number<input type="tel" inputMode="tel" autoComplete="tel" value={customerPhone} maxLength={40} onChange={event => setCustomerPhone(event.target.value)} placeholder="Optional"/></label><label>Shop name<input autoComplete="organization" value={shopName} maxLength={120} onChange={event => setShopName(event.target.value)} placeholder="Optional"/></label><label>Address<input autoComplete="street-address" value={customerAddress} maxLength={500} onChange={event => setCustomerAddress(event.target.value)} placeholder="Optional"/></label>
        <label>Discount percentage<div className="discount-input"><NumberInput inputMode="decimal" min="0" max="100" step="0.01" value={discount} onChange={event => setDiscount(event.target.value)}/><span>%</span></div></label>
