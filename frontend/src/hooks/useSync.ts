...
export function useSync(){
  ...
  useEffect(()=>{
    let debounceTimer:number|undefined,retryTimer:number|undefined,retryDelay=1_000,stopped=false;
    const run=async()=>{
    ...
    const changed=()=>{clearTimeout(debounceTimer);debounceTimer=window.setTimeout(run,50)};
    ...
    return()=>{stopped=true;channel?.close();removeEventListener('online',run);removeEventListener('offline',off);removeEventListener('focus',run);removeEventListener('konooz:data-changed',changed);removeEventListener('konooz:sync-request',run);document.removeEventListener('visibilitychange',visible);clearInterval(interval);clearTimeout(debounceTimer);clearTimeout(retryTimer)};
  },[]);
