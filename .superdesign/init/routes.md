...
The app uses React Router with a public login route and four authenticated routes nested beneath the shared Shell.
...
function Guard(){
  ...
  if(authenticated===null)return <div className="route-loading" role="status">Checking secure sessionâ€¦</div>;
  return authenticated?<Shell/>:<Navigate to="/login" replace/>;
}
export default function App(){
  return <BrowserRouter><Suspense fallback={<div className="route-loading" role="status">Loadingâ€¦</div>}><Routes>
